...
validateInput = function() {
    ...
    if (!validateField(validateName, 'first-name-error', 'first-name'))
        fieldsValid = false;

    if (!validateField(validateName, 'last-name-error', 'last-name'))
        fieldsValid = false;

    if (!validateField(validateEmailAddress, 'email-address-error', 'email-address'))
        fieldsValid = false;

    if (!validateField(validateUserName, 'user-name-error', 'user-name'))
        fieldsValid = false;

    if (!validateField(validatePassword, 'password-error', 'password'))
        fieldsValid = false;

    if (!validateField(validateConfirmPassword, 'password-confirm-error', 'password', 'password-confirm'))
        fieldsValid = false;
